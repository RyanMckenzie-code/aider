...
"""
    ...
    def cmd_supervise(self, args):
        "Toggle supervised mode for edit approvals. Usage: /supervise [on|off]"
        arg = (args or "").strip().lower()
        if not arg:
            self.coder.supervised_mode = not self.coder.supervised_mode
        elif arg in ("on", "true", "yes", "1"):
            self.coder.supervised_mode = True
        elif arg in ("off", "false", "no", "0"):
            self.coder.supervised_mode = False
        else:
            self.io.tool_error("Usage: /supervise [on|off]")
            return

        mode = "ON" if self.coder.supervised_mode else "OFF"
        self.io.tool_output(f"Supervised mode is now {mode}.")
