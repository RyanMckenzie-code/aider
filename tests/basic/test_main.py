...
class TestMain(TestCase):
    ...
    def test_supervised_mode_argument_is_available(self):
    ...
    def test_supervised_mode_passed_to_coder(self):
        with patch("aider.coders.Coder.create") as MockCoder:
            main(
            ...
            MockCoder.assert_called_once()
            _, kwargs = MockCoder.call_args
            self.assertEqual(kwargs["supervised_mode"], True)

    def test_supervised_mode_false_by_default(self):
        with patch("aider.coders.Coder.create") as MockCoder:
            main(["--no-git", "--yes"], input=DummyInput(), output=DummyOutput())
            MockCoder.assert_called_once()
            _, kwargs = MockCoder.call_args
            self.assertEqual(kwargs["supervised_mode"], False)

    def test_supervised_mode_from_env_file(self):
        self.create_env_file(".env", "AIDER_SUPERVISED_MODE=on")
        with patch("aider.coders.Coder.create") as MockCoder:
            main(["--no-git", "--yes"], input=DummyInput(), output=DummyOutput())
            MockCoder.assert_called_once()
            _, kwargs = MockCoder.call_args
